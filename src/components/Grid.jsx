...
const Grid = ({ gridSize, isHalfGrid, spline, depots, onCellClick, onNodeClick, onRotateDepot, onDepotMouseDown, draggedDepotId, hoveredDepotId, onMouseDown, selectedTool, carColors, activeTab, selectedDepotId, setSelectedDepotId, onCarClick, selectedCarColor }) => {
  ...
  const generateSplinePath = () => {
    ...
    const points = spline.nodes.map(n => ({
      ...
      y: (gridSize.height - n.z - 0.5) * CELL_SIZE + nodeOffset
    }));
      ...
      >
        ...
        return (
          <div 
            ...
            onMouseDown={() => onCellClick(cell.x, gridSize.height - cell.y - 0.5, 'down')}
            onMouseEnter={() => onCellClick(cell.x, gridSize.height - cell.y - 0.5, 'enter')}
            >
            </div>
          );
        })}
        ...
        {spline?.nodes?.map((node, i) => (
          <g key={`node-group-${i}`}>
            <circle 
              ...
              cy={(gridSize.height - node.z - 0.5) * CELL_SIZE + nodeOffset} 
              r={8} 
            ...
            <text 
              ...
              y={(gridSize.height - node.z - 0.5) * CELL_SIZE + nodeOffset - 12}
              fill="white"
              fontSize="12"
              fontWeight="bold"
              textAnchor="middle"
